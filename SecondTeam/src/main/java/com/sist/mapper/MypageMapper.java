...
public interface MypageMapper {
	...
	@Select("SELECT title,centername,TO_CHAR(v_start,'YYYY-MM-DD') as dbV_start,TO_CHAR(v_end,'YYYY-MM-DD') as dbV_end,"
			+"collect_state,vano,v_state,TO_CHAR(v_state_time,'YYYY-MM-DD HH24:MI:SS') as vDbStateTime,num "
			+"FROM(SELECT title,centername,v_start,v_end,collect_state,vano,v_state,v_state_time,rownum as num "
			+"FROM(SELECT title,centername,v_start,v_end,collect_state,vano,v_state,v_state_time "
			+"FROM v_program_apply a "
			+"JOIN V_PROGRAM b "
			+"ON a.vno=b.vno "
			+"WHERE id=#{id} ORDER BY vano DESC)) "
