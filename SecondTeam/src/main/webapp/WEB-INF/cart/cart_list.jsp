...
<div class="container" id="cartApp">
						...
						<tbody>
							...
							<tr class="hideAndShow">
								<td class="shoping__cart__item"><a
									:href="'../goods/goods_detail.do?gno='+vo.gvo.gno"><img
										:src="vo.gvo.g_img" class="shoppingItemImage"></a>
									<h5 style="display: inline; font-weight: bold;">{{vo.gvo.g_name}}</h5>
								</td>
								<td><h7 class="hs_original-price">{{vo.gvo.g_price}}</h7> <h7
										class="hs_discount-rate"></h7>
									<h4 class="hs_final-price getShopPrice"></h4></td>
								<td class="shoping__cart__quantity">
									<div class="kyj_shoppingQuantity">
										<div class="kyj__ShopInput-container">
											<button class="kyj_shoppingDecreseBtn"
												@click="decreaseQuantity(index)">-</button>


											<input type="text" :value="vo.cart_count"
												class="kyj_shoppingCal" id="quantity" refs="quantity"
												v-model="vo.cart_count" min="1" readonly>
											<button class="kyj_shoppingIncreseBtn"
												@click="increaseQuantity(index)">+</button>
										</div>
									</div>
								</td>
								<td class="shoping__cart__total"><span v-if="!modify">{{(vo.cart_price*1).toLocaleString()}}</span>
		...
		<th scope="row">주문금액</th>
				<td>W<span class="prd_prc_cart_87" v-if="!modify">{{totalPrice.toLocaleString()}}</span></td>
		</tr>
			<tr>
        		...
        		cartData(){
        			axios.get("../cart/cart_list_vue.do",{
        				...
        				for (let i = 0; i < this.cart_list.length; i++) {
            	        	this.buy_state= this.cart_list[i].buy_state;
            	        }
        				console.log("bs:"+this.buy_state)
        				this.TotalPrice()
